...
`prompts/registry.json` lists approved project prompt versions. It starts empty: installation does not invent specialized skills or evaluation results. Each entry records `id`, stable `major.minor.patch` version, `path` equal to `prompts/<id>/<version>/SKILL.md`, SHA-256 of the exact file bytes, accountable `owner` and `evaluationRef`.
...
Keep approved versions immutable and point consumers at an explicit version. `gated-pipeline governance` catches changed bytes, missing/linked files and duplicate id/version entries and shared or unversioned paths. It cannot detect a person changing both an old fingerprint and its file in the same PR; independent Git review must enforce version history and assess the evaluation. A hash is not a signature or proof the prompt is good.
