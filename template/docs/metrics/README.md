...
Use `docs/traces/` and existing evidence for gate outcomes; usage receipts and cost-policy checks support cost reporting. The scaffold supplies definitions, record validation and the local `calibrate` command; it does not collect telemetry or calculate ROI. Calibration groups declared task/risk/route/billing cohorts, retains failed attempts and reports missing measurements without attributing mixed-unit outcomes to individual models. Capture a measured baseline before claiming improvement, and review these results at the project's cadence boundaries.
