...
**Within-gate fan-out** (CORE cross-gate rule): gate 5 is eligible. MAY spawn concurrent dimension-checkers (correctness / security-smell / simplicity) synthesized into one verdict + one exit block. Bounds: surface non-collision, both exit before synthesis, finding attribution recorded. Judgment, not mandate — a small diff does not fan out.
...
**Hunt the dimensions in [`.claude/rules/checklists/code-review.md`](../checklists/code-review.md)** (canonical home — logic/edges, core-purity & other invariants, TDS drift, vacuous tests). Fan-out dimension-checkers read the same checklist.
...
**Rework rounds** (CORE rule): non-BLOCKING findings are fixed in-branch by the owning earlier-gate agent; `FAIL` + loop-back reserved for BLOCKING or wrong-direction findings. FAIL rounds recorded unsanitized.
