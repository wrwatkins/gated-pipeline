...
Project rule file (ADR-014; the conduct subset of former `the project's earlier instructions file` §Security & deploy — these bind every session, not just IaC diffs; BR-008 D6). Unconditional — loads every session. IaC/deploy rules live in `.claude/rules/security-deploy.md` (path-scoped).
