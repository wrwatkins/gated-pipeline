...
- All new and updated logic is covered by automated tests **at its appropriate tier** — unit for pure logic, integration for route handlers, E2E for flows (PROCESS §Test tiers; supersedes the earlier blanket unit-test wording — DEBT D9. The int-tier-first practice for handlers was always the sanctioned reading).
- Run a SAST scan after every task.
- Lint, typecheck, and tests green before any PR (PROCESS §Scans — all blocking).
- The pre-PR completion bar is the shared **Definition of Done** checklist — [`.claude/rules/checklists/definition-of-done.md`](checklists/definition-of-done.md) (ADR-024).
