...
Project rule file (ADR-014; the IaC/deploy subset of former `docs/AI-INSTRUCTIONS.md` §Security & deploy — BR-008 D6 split). Loads when IaC/workflow/container files are worked on. Session-wide conduct rules (secrets, logs, no-"barely-covers") live in `.claude/rules/security-conduct.md` (unconditional).
...
- Encryption at rest and in transit is mandatory on every applicable resource — the concrete resource checklist lives in `.claude/skills/pr-review/SKILL.md` §Universal infrastructure checklist (Terraform / IaC).
