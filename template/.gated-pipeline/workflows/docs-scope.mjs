...
export function inspectDocsDiff(directory, { base, head } = {}) {
  ...
  const raw = git(['diff', '--no-ext-diff', '--no-textconv', '--no-renames', '--raw', '-z', base, head, '--']).split('\0')
  const paths = [], reasons = []
  ...
  try { git(['diff', '--no-ext-diff', '--no-textconv', '--check', base, head, '--']) }
  catch { whitespaceClean = false; reasons.push('Complete committed diff fails whitespace verification') }
  const diff = git(['diff', '--no-ext-diff', '--no-textconv', '--no-renames', '--binary', base, head, '--'])
