...
const LEGACY_HOOK_PATH = '.claude/hooks/block-push-to-main.sh'
const legacyHookCommands = new Set([
  LEGACY_HOOK_PATH, `"${LEGACY_HOOK_PATH}"`, `'${LEGACY_HOOK_PATH}'`,
  `"$CLAUDE_PROJECT_DIR/${LEGACY_HOOK_PATH}"`, `"${'${CLAUDE_PROJECT_DIR}'}/${LEGACY_HOOK_PATH}"`,
].flatMap(path => [path, `sh ${path}`, `bash ${path}`]))
...
export function mergeSettings(previous, { retireLegacyHook = false, enableGuard = true } = {}) {
  ...
  if (retireLegacyHook) {
    const recognized = hook => hook?.type === 'command' && typeof hook.command === 'string' && legacyHookCommands.has(hook.command.trim())
    settings.hooks.PreToolUse = settings.hooks.PreToolUse.flatMap(group => {
      if (group?.matcher !== 'Bash' || !Array.isArray(group.hooks) || !group.hooks.some(recognized)) return [group]
      const hooks = group.hooks.flatMap(hook => recognized(hook)
        ? enableGuard ? [{ ...hook, command: HOOK_COMMAND }] : [] : [hook])
      return hooks.length ? [{ ...group, hooks }] : []
    })
    if (JSON.stringify(settings.hooks).includes(LEGACY_HOOK_PATH)) {
      throw new Error('Unrecognized legacy hook reference; reconcile settings or protect the legacy hook script before sync. No files changed.')
    }
  }
  ...
  if (enableGuard && !registered) settings.hooks.PreToolUse.push({ matcher: 'Bash', hooks: [{ type: 'command', command: HOOK_COMMAND }] })
...
export async function scaffold(dest, { command, tokens = defaults, adapters, dry = false } = {}) {
  ...
  const retireLegacyHook = plan.some(change => change.path === LEGACY_HOOK_PATH && change.text === null)
  const enableGuard = cfg.adapters.includes('claude')
  if (retireLegacyHook) {
    const localSettings = await read(await safePath(dest, '.claude/settings.local.json'))
    if (localSettings !== null && JSON.stringify(JSON.parse(localSettings)?.hooks ?? {}).includes(LEGACY_HOOK_PATH)) {
      throw new Error('Project-local settings reference the retiring legacy hook; reconcile settings.local.json or protect its script before sync. No files changed.')
    }
  }
  if (enableGuard || retireLegacyHook) {
    const path = '.claude/settings.json'
    const previous = await read(await safePath(dest, path))
    if (protectedPath(path)) {
      if (retireLegacyHook && previous !== null && JSON.stringify(JSON.parse(previous)?.hooks ?? {}).includes(LEGACY_HOOK_PATH)) {
        throw new Error('Protected settings still reference the retiring legacy hook; protect its script or reconcile settings before sync. No files changed.')
      }
    } else if (enableGuard || previous !== null) {
      await schedule(path, mergeSettings(previous, { retireLegacyHook, enableGuard }))
    }
  }
