...
import { atomicWrite, read } from './files.mjs'
export async function gitHooks(dest,{check=false}={}) {
  ...
  if(previous!==null&&(await lstat(path)).isSymbolicLink()) throw new Error('Existing pre-push hook is a symlink; integrate manually')
