...
export function checkEvidence(record, project, { head, through = 9 } = {}) {
  ...
  for (const [index, entry] of record.attempts.entries()) {
    ...
    for (let gate = 1; gate < entry.gate; gate++) {
  ...
  const latest = []
  for (let gate = 1; gate <= through; gate++) {
    ...
    let required = gate === 4 ? project.requiredChecks : gate === 6 ? ['test', ...(project.commands.e2e ? ['e2e'] : [])] : gate === 7 ? ['sast', 'audit'].filter(name => project.commands[name]) : gate === 8 ? ['rollback', ...project.ci.requiredChecks] : []
    for (const name of required) {
      ...
      if (gate === 8 && project.ci.requiredChecks.includes(name) && item?.source !== 'ci') errors.push(`Gate 8: ${name} must cite CI evidence`)
    }
  }
  for (const name of project.requiredChecks) if (!project.commands[name]) errors.push(`Project required command ${name} is not configured`)
  if (project.independentReview && through >= 5) {
    const author = states.get(4)?.actor
    for (const gate of [5, 7, 9].filter(gate => gate <= through)) {
...
export function prBody(record, latest) {
  const rows = latest.map(entry => `| ${entry.gate} · ${roleNames[entry.gate-1]} | ${cell(entry.actor.tool)} | ${cell(entry.actor.id)} | ${cell(entry.actor.model)} | ${cell(entry.actor.runId)} | ${entry.result} |`)
